  ...
  describe "list-extensions" do
    it "lists available extensions for a type" do
      ...
      expect(output).to include(
        "Supported extensions: xml, presentation, rxl, html",
      )

      command = %w(list-extensions icc)
      output = capture_stdout { Metanorma::Cli.start(command) }
      expect(output).to include(
        "Supported extensions: xml, html, pdf, doc and presentation",
      )
      expect(output).to include("Base flavor: iso")
      expect(output).to include(
        "Flavor extensions: xml, presentation, rxl, html",
      )
    end
    ...
    it "lists all extensions if no type specified" do
      ...
      expect(output).to include(
        "icc (base flavor: iso): xml, html, pdf, doc and presentation. " \
        "(Flavor extensions: xml, presentation, rxl, html, html_alt, " \
        "doc, pdf, sts and isosts).",
      )
    end
